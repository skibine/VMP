...
<div class="h-full overflow-auto p-4 space-y-4">
  <div class="hud-panel p-4 space-y-3">
    <div class="flex items-center gap-2">
      <h2 class="font-mono text-neon-green text-lg">domains&nbsp;//&nbsp;{domains.length}</h2>
    </div>
    <form on:submit|preventDefault={add} class="flex items-center gap-2">
      ...
      <button class="hud-btn hud-btn-primary" disabled={busy}>{busy ? '…' : '+ add domain'}</button>
    </form>
    {#if err}<div class="text-xs font-mono text-neon-red">{err}</div>{/if}
    {#if !domains.length}
      <div class="hud-label text-hud-dim">// no domains — add one to track its DNS, cert expiry and whois age.</div>
    {:else}
      <div class="space-y-1">
        {#each domains as d (d.id)}
          <div class="flex items-center gap-2 border border-hud-line rounded px-2 py-1.5">
            ...
            <button class="hud-btn !py-0.5" on:click={() => probe(d)} disabled={infoBusy && selected === d.id}>{infoBusy && selected === d.id ? '…' : 'probe'}</button>
  ...
  {#if selected}
    <section class="hud-panel p-4 space-y-3">
      <div class="flex items-center gap-2">
        <span class="hud-label text-neon-cyan">domain&nbsp;//&nbsp;info</span>
        <span class="font-mono text-emerald-200 ml-1">{domains.find((d) => d.id === selected)?.name}</span>
      </div>
      {#if infoBusy}
        <div class="hud-label text-hud-dim animate-pulse">probing dns · cert · whois…</div>
      {:else if info?.error}
        <div class="text-xs font-mono text-neon-red">{info.error}</div>
      {:else if info}
        <!-- Certificate -->
        <div>
          <div class="hud-label text-hud-dim mb-1">certificate</div>
          {#if info.cert?.present}
            <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs font-mono">
              ...
              <div><span class="hud-label text-hud-dim">status</span> <span class={certColor(info.cert.status)}>{info.cert.status}</span></div>
            </div>
          {:else}
            <div class="text-xs font-mono text-hud-dim">no TLS / port 443 closed</div>
          {/if}
        </div>
        <!-- DNS records -->
        <div>
          <div class="hud-label text-hud-dim mb-1">dns</div>
          <div class="text-xs font-mono space-y-0.5">
            ...
            {#if !info.dns?.a?.length && !info.dns?.ns?.length}<div class="text-hud-dim">no records</div>{/if}
          </div>
        </div>
        <!-- Whois -->
        <div>
          <div class="hud-label text-hud-dim mb-1">whois</div>
          <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs font-mono">
            ...
            <div><span class="hud-label text-hud-dim">status</span> {info.whois?.status || '—'}</div>
          </div>
